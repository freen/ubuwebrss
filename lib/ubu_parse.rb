...
class UbuParse
	...
	private
	def _extract_addition_item_after_image( image_node )
		...
		entry = {
			:artist => '',
			:title => '',
			:href => _normalize_href( anchor_node[:href] )
		}
	...
	def _extract_addition_items_after_heading( header_node )
		items = []
		...
		while 1
			...
			when 'br'
				next
			when 'img'
				item = _extract_addition_item_after_image( next_node )
	...
	def _extract_recursive_collection_entries( font_node )
		collection_entries = []
		...
		font_node.children.each do |child_node|
			...
			when 'b'
				...
				if title_anchor and title_anchor.name == 'a'
					...
					entry[:href] = _normalize_href( title_anchor[:href] )
			...
			when 'font'
				...
				collection_entries += _extract_recursive_collection_entries( child_node )
			end
		end
	...
	# in UbuWeb's markup.)
	def _extract_addition_entries( column_node , header_xpath)
		heading = column_node.xpath( header_xpath ).first
		return false if heading.nil?
		return _extract_addition_items_after_heading( heading )
	end
	...
	def extract
		...
		columns = @doc.xpath( "//td[@class='default']" ).each_with_index do |column, index|
			...
			if additions_column_metadata.has_key? index
				...
				content_sets[slug] = _extract_addition_entries( column , header_xpath )

			# Process center column uniquely
			elsif index == 1
				content_sets[:collection_announcements] = _extract_collection_entries( column )
			end
